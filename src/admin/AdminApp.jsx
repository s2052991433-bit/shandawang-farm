...
function ProductEditor({ product, close, save }) {
  ...
  return <div className="admin-drawer-wrap"><button className="admin-drawer-backdrop" onClick={close} aria-label="关闭" /><aside className="admin-drawer"><header><div><p>商品资料</p><h2>{product.name}</h2></div><button onClick={close}><X /></button></header><form onSubmit={(event) => { event.preventDefault(); const labels = { "farm-grown": "农场自产", vegetables: "时令时蔬", "ningbo-select": "宁波精选", "ningbo-specialty": "宁波特产", "gift-card": "礼品卡" }; const seasonLabels = { spring: "春", summer: "夏", autumn: "秋", winter: "冬", annual: "全年" }; save({ ...form, categoryLabel: labels[form.category] || form.categoryLabel, seasonLabel: seasonLabels[form.season] || "夏", price: Number(form.price), inventory: Number(form.inventory), sortOrder: Number(form.sortOrder) }); }}>
    ...
    <div className="admin-field-grid"><label><span>时令归属</span><select value={form.season || "summer"} onChange={update("season")}><option value="spring">春季 · 2—4月</option><option value="summer">夏季 · 5—8月</option><option value="autumn">秋季 · 9—10月</option><option value="winter">冬季 · 11—次年1月</option><option value="annual">全年禽蛋 / 年卡</option></select></label><label><span>销售方式</span><input value={form.saleMode === "available" ? "当季正常售卖" : "非当季预售"} disabled /></label></div>
...
function VoucherImporter({ close, imported }) {
  ...
  return <div className="admin-drawer-wrap"><button className="admin-drawer-backdrop" disabled={working} onClick={close} aria-label="关闭" /><aside className="admin-drawer admin-voucher-drawer"><header><div><p>保留原编号与卡密</p><h2>导入鸡蛋年卡</h2></div><button disabled={working} onClick={close} aria-label="关闭导入"><X /></button></header><form onSubmit={submit}>
    <div className="annual-rule-note"><Egg /><div><strong>2027年鸡蛋年卡 · ¥798</strong><span>1月至12月，每月30枚，共360枚。客户只需输入卡密，编号仅供后台识别。</span></div></div>
...
function Vouchers({ vouchers, setVouchers, notify, refresh }) {
  ...
  return <>
    ...
    <section className="admin-table-card">{vouchers.length ? <div className="admin-table admin-voucher-table"><div className="admin-table-head"><span>卡券</span><span>卡密尾号</span><span>面值</span><span>状态</span><span>有效期</span></div>{vouchers.map((voucher) => <div className="admin-table-row" key={voucher.id}><span><strong>{voucher.name}</strong><small>{voucher.cardNumber ? `编号 ${voucher.cardNumber}` : voucher.type === "annual_card" ? "鸡蛋年卡" : "余额礼卡"}</small></span><span>•••• {voucher.codeHint}</span><span>{money(voucher.value)}</span><StatusPill value={voucher.status} labels={{ active: "可使用", activated: "已激活", used: "已用完", disabled: "已停用" }} /><span>{voucher.expiresAt || "长期有效"}</span></div>)}</div> : <EmptyState icon={Ticket} title="还没有卡券" text="已有实体卡请选择“导入已有年卡”，保留原卡号和密码。" />}</section>
