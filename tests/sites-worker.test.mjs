...
test("imports printed annual cards atomically, preserves leading zeroes, and activates twelve deliveries once", async () => {
  ...
  const cards = [{ cardNumber: "NO.001", password: "0123456" }, { cardNumber: "NO.002", password: "0765432" }];
  ...
  const conflict = await call("/api/admin/vouchers/import", { cards: [{ cardNumber: "NO.003", password: "1111111" }, { cardNumber: "NO.001", password: "2222222" }] }, cookie);
  assert.equal(conflict.status, 409);
  assert.equal((await env.DB.prepare("SELECT COUNT(*) AS n FROM vouchers").first()).n, 2);
  assert.equal((await call("/api/vouchers/validate", { code: "WRONGCODE" })).status, 404);
  assert.equal((await call("/api/vouchers/validate", { code: "0123456" })).status, 200);
  const checked = await (await call("/api/vouchers/validate", { code: "0123456" })).json();
  ...
  const body = { voucherCode: "0123456", address: { receiver: "测试", phone: "13800138000", province: "浙江省", city: "宁波市", district: "鄞州区", detail: "测试路1号" } };
  assert.equal((await call("/api/redemptions", { ...body, voucherCode: "WRONGCODE" })).status, 409);
  ...
  assert.equal((await call("/api/redemptions", { ...body, voucherCode: "0765432", })).status, 410);
...
test("parses spreadsheet columns without losing zeroes and rejects duplicate pairs", async () => {
  ...
  assert.deepEqual(parseCardText("卡号\t密码\nNO.001\t0123456"), [{ cardNumber: "NO.001", password: "0123456" }]);
  assert.deepEqual(parseCardText('\uFEFF"卡号","密码"\r\n"NO.001","0123456"'), [{ cardNumber: "NO.001", password: "0123456" }]);
  assert.throws(() => parseCardText("NO.001\t0123456\nNO.001\t9999999"), /重复/);
  assert.throws(() => parseCardText("NO.001\t0123456\t额外列"), /两列/);
